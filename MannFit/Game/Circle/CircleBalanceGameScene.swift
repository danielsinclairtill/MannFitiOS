...
class CircleBalanceGameScene: SKScene {
    ...
    override func update(_ currentTime: CFTimeInterval) {
        ...
        if let data = motionManager.accelerometerData {
            ...
            player.position.y = CGFloat(smoothYAcceleration.value - playerCenterY) * frame.height / 2 * CGFloat(sensitivity) + frame.height / 2
        }
    ...
    @objc private func gameOver(completed: Bool) {
        ...
        self.engine?.stop()
        if completed {
            self.gameOverDelegate?.sendGameData(game: "Circle Balance", duration: Int(exerciseTime), absement: Float(absementScore))
        }
